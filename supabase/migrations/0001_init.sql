-- Atlas AI — initial schema
-- Run this once in the Supabase SQL Editor (or via `supabase db push` if you
-- have the project linked with the Supabase CLI).
--
-- Supabase already manages authenticated users in the built-in `auth.users`
-- table. This migration adds a `public.profiles` table that mirrors each
-- user with app-editable fields, kept in sync via a trigger.
